...
get_cor <- function(df = df, v = "CS", v_ref = "Ref"){
  v     <- df[[v]]
  v_ref <- df[[v_ref]]
...
load(file = here::here("data-raw", "blags_1km.RData"), verbose = FALSE)
```

```{r, plotting, eval = FALSE, echo = FALSE}
# dA time series
p <- ggplot(dt_dA_obs[time > 1990 & time < 2020 & area != set_units(0, km^2)], 
...
df <- pivot_wider(dt_dA_obs, names_from = data_source, values_from = area)
df <- subset(df, time >= 1990 & time <= 2020)
df$Ref <- df$AgCensus
df$Ref[df$u == "woods"] <- df$FC[df$u == "woods"]
v_names_sources = c("AgCensus", "CS", "LCM", "CORINE", "LCC", "IACS")
v_rmse <- sapply(v_names_sources, getRMSE, df = df, v_ref = "Ref")
v_r2 <- sapply(v_names_sources, get_cor,     df = df, v_ref = "Ref")
# df_scaled <- data.frame(RMSE = v_rmse, invr2 = 1/(v_r2+1))
# df_scaled <- as.data.frame(scale(df_scaled))
# df_scaled$sigma_scaling <-  df_scaled$RMSE * df_scaled$invr2
# df_scaled$sigma_scaling <-  df_scaled$sigma_scaling

df_scaled <- data.frame(RMSE = v_rmse, invr2 = 1/(v_r2+1))

df_out <- data.frame(RMSE = v_rmse, r2 = v_r2, 
  sigma_scaling = v_rmse * 1/(v_r2+1))

df_out <- df_out[order(df_out$sigma_scaling),]
df_out <- df_out[-1,] # remove AgCensus, as most of ref data
knitr::kable(df_out)
```
