...
l_df_AgCensus_Eng  <- tar_read(c_agc_en)
l_df_AgCensus_Sco  <- tar_read(c_agc_sc)
l_df_AgCensus_Wal  <- tar_read(c_agc_wa)
l_df_AgCensus_NIr  <- tar_read(c_agc_ni)
...
p <- ggplot(l_df_AgCensus_Eng$df_long, aes(time, area))
...
p <- ggplot(l_df_AgCensus_Sco$df_long, aes(time, area))
# p <- p + geom_vline (xintercept =   knownChangePoint, colour = "green")
# p <- p + geom_vline (xintercept = unknownChangePoint, colour = "red")
p <- p + geom_line()
...
p <- ggplot(l_df_AgCensus_Wal$df_long, aes(time, area))
p <- p + geom_line()
p <- p + geom_point()
...
p <- ggplot(l_df_AgCensus_NIr$df_long, aes(time, area))
p <- p + geom_line()
p <- p + geom_point()
p <- p + xlim(1981, 2020)
p + facet_wrap(~ u, scales = "free_y")
