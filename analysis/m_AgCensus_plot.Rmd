...
dt_A_UK  <- tar_read(c_dA_AgCensus)$dt_A_UK
dt_dA_UK <- tar_read(c_dA_AgCensus)$dt_dA_UK
```
...
```{r plot_dA_UK, eval=TRUE, echo=FALSE, warning=FALSE, message=FALSE, fig.cap = "Time series of net land-use change from the June Agricultural Census in the UK, 1980-2019."}
p <- ggplot(dt_dA_UK, aes(time, area))
p <- p + geom_line()
