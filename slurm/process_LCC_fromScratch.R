...
fname <- here("data-raw/LCC", "LCC_to_LULUCF.csv")
dt_u_lcc_lulucf <- fread(fname)
...
reclassify_LCC <- function(year, dt_u_lcc_lulucf, r_25m){
  ...
  if(year == 2015){
    v_U <- sf::st_read(dsn = here("data-raw/LCC", year), 
                       layer = "cropmap_2015_with_status_170315")
  } else {
    v_U <- sf::st_read(dsn = here("data-raw/LCC", year), 
                       layer = paste0("crops", year, "_GB"))
  }
    ...
    writeRaster(r_U_lcc, fname, overwrite=T)
...
r_U_lcc_25m <- reclassify_LCC(year, dt_u_lcc_lulucf, r_25m)
